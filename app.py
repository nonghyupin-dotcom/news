    ...
    with tab2:
        ...
        if not news_ws:
            ...
            if df.empty:
                ...
                with col_chart1:
                    st.markdown("**🔍 키워드별 수집량**")
                    if '키워드' in df.columns:
                        kw_counts = df['키워드'].value_counts()
                        st.bar_chart(kw_counts)
                        
                with col_chart2:
                    st.markdown("**🏢 언론사별 기사 발행량 (TOP 10)**")
        ...
        if board_ws:
            ...
            try:
                ...
                if len(board_values) > 1:
                    for i in range(len(board_values)-1, 0, -1):
                        r = board_values[i]
                        sheet_row_index = i + 1
                        ...
                        with col_msg:
                            ...
                            if b_content:
                                st.info(b_content)
                        with col_btn:
                            if st.button("❌", key=f"del_{sheet_row_index}", help="이 의견 삭제하기"):
                                board_ws.delete_row(sheet_row_index)
                                st.rerun()
                else:
